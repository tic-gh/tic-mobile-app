...
const openCamera = async (picture: Picture) => {
  try {
    ...
    if (action === 'take-picture') {
      if (picture.remark && picture.remark == 'N/A') {
        picture.remark = '';
      }
...
const addNewPicture = () => {
  const newPicture: Picture = {
    type: 'multi-take-picture-remarks',
    label: props.label ?? '',
    value: '',
    remark: '',
  };
...
const isComplete = computed(() => {
  ...
  return data.value.every((picture) => {
    ...
    // Check if remark is provided (either has content or is marked as "N/A")
    const hasRemark = picture.remark && (picture.remark.trim() !== '' || picture.remark === 'N/A');

    // If editable is enabled, also check for label
    if (props.options?.editable) {
      const hasLabel = picture.label && picture.label.trim() !== '';
      return hasValue && hasLabel && hasRemark;
    }

    return hasValue && hasRemark;
  });
});
...
<template>
  <ion-grid class="multi-take-picture-remarks ion-no-padding ion-margin-bottom">
    <ion-row>
      <ion-col>
        <section v-for="(picture, i) in data" :key="`${id}-${i}`" class="section">
          ...
          <div class="remark-container">
            <ion-label class="form-input-label">{{ t('picture_remark') }}:</ion-label>
            <ion-textarea
              :disabled="picture.remark == 'N/A'"
              autocomplete="on"
              autocorrect="on"
              autocapitalize="on"
              :spellcheck="true"
              :auto-grow="true"
              v-model="picture.remark"
              @ionChange="debouncedSaveData"
              fill="solid">
            </ion-textarea>
          </div>
